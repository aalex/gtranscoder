...
void run_main_window()
{
    ...
    g_signal_connect(quit_menu_item, "activate", G_CALLBACK(end_program), NULL);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(menu_bar), file_menu_widget);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(menu_bar), help_menu_widget);
